...
class App
  ...
  def route_list
    ...
    proc do |wagon, index|
      index += 1
      info = "#{index}. Wagon №#{wagon.number}. Type: #{wagon.class.to_s}."
  ...
  def train_info
    proc do |train, index|
      index += 1
      puts "#{index}. Train №#{train.number}. Type: #{train.class.to_s}. Amount of railcars: #{train.wagons.size}"
    end
  end

  def wagon_action(type) # метод для отбора пассажирских и грузовых поездов
    station_list
    ...
    wrong_number if @railcar < 0 || @wagon > @trains.size
  end  
